...
if (TURSO_DATABASE_URL && TURSO_AUTH_TOKEN) {
  try {
    ...
    console.error("ERROR: No se pudo inicializar el cliente de Turso. Verifique las variables de entorno o la conexión.", e);
    // Asegurarse de que tursoClient permanezca null si falla la inicialización
    tursoClient = null;
  }
} else {
  console.warn("ADVERTENCIA: Variables de entorno de Turso (TURSO_DATABASE_URL o TURSO_AUTH_TOKEN) no están configuradas. Los datos NO se guardarán en la base de datos.");
}
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!validationResult.success) {
      ...
      console.error("Error de validación del formulario de contacto:", validationResult.error.errors);
      return new Response(
        JSON.stringify({
          status: "error",
    ...
    // --- 1. Guardar en Turso ---
    if (tursoClient) { // TypeScript ahora sabe que tursoClient es de tipo Client aquí
      try {
        await tursoClient.execute({
          sql: `
            INSERT INTO form_contacto (name, email, subject, message)
            VALUES (?, ?, ?, ?)
          `,
      ...
      console.warn("Cliente de Turso no inicializado. Los datos de contacto NO se guardaron en la base de datos.");
    }
    ...
    if (!RESEND_API_KEY || !TARGET_EMAIL || !FROM_EMAIL) {
        console.error("Faltan variables de entorno cruciales para el envío de correo. No se puede enviar el email.");
        // Si la DB falla, y el email también por falta de config, esto sí es un error crítico
        return new Response(
            JSON.stringify({
                status: "error",
                message: "Error de configuración del servidor. Contacte al administrador. (Faltan variables de Resend)",
            }),
            {
    ...
    try {
      ...
      console.log(`Mensaje de contacto recibido de ${name} <${email}>. Asunto: ${subject}`);
    } catch (emailError) {
    ...
    console.error("Error general al procesar el formulario de contacto:", error);
